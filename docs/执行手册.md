...
### 4.5 训练与推理脚本
- `train.py` 标准流程（当前实现）：
  1. 解析命令行参数（默认读取 `data/processed/` 最新特征，可通过 `--processed_dir` 覆盖）。
  2. 加载 `train_l1.parquet`/`test_l1.parquet`，执行 z-score 截断、TimeSeriesSplit、LightGBM + Ridge 集成训练。
  3. 在 `logs/experiments/<timestamp>_<name>_<hash>/` 下保存配置、指标、OOF 预测、测试预测、特征重要性及每折模型（LightGBM `.txt` + Ridge `.joblib`），并生成 `submission.csv`。
- `predict.py`：
  1. 指定 `--experiment logs/experiments/<...>`，脚本解析 `config.json` 自动定位特征目录。
  2. 复用保存的每折模型与权重生成预测，默认写入 `<experiment>/submission_infer.csv`（可通过 `--out_sub` 覆盖）。
  3. 推理前会重新执行 z-score 截断与缺失检查，确保与训练阶段一致。
